...
cmd /c util\wash.cmd blob download "HelloSong.war" "HelloSong.war" eclipsedeploy sijojapan "Au3eiexRvDPG0KDIyJdkSlbQnWNJ75bkMITWzLSW+bvgS7+xxp60/UopvtNuCazdH9zto0nGorb/eIc15OhHgw==" "https://core.windows.net"
if not exist "HelloSong.war" exit 0
if not "%SERVER_APPS_LOCATION%" == "\%ROLENAME%" if exist "HelloSong.war"\* (echo d | xcopy /y /e /q "HelloSong.war" "%SERVER_APPS_LOCATION%\HelloSong.war" 1>nul) else (echo f | xcopy /y /q "HelloSong.war" "%SERVER_APPS_LOCATION%\HelloSong.war" 1>nul)
